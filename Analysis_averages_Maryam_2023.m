...
path2data = 'C:\Users\Anacker1\Desktop\Fiber-Data\Results\Y maze\Day1\'; %Location of the data
path2savefolder = 'C:\Users\Anacker1\Desktop\Fiber-Data\Results\Y maze\Day1\'; %Path to save
...
IdChannel = {'dCA1GCamp6f','vCA1GCamp6f'}; %%%%{'GCamp6f',[],'Psychlight'}vCA1GCamp6f dCA1GCamp6f
color2plotchannels = {'r','g'};
...
LimitXaxis = [-5 5]; % Window to visualize in the plot. If empty, full length of TRANGE
...
if ~isempty(all_mice_trials)
    Outcome = fieldnames(all_mice_trials);
    if ~isempty(all_mice_trials.(Outcome{1}))
        Condition = fieldnames(all_mice_trials.(Outcome{1}));
    else
        Condition = fieldnames(all_mice_trials.(Outcome{2}));
    end
    avg_mode = 1; % 1, use mean and SEM; 2, use median and MAD
    for cond = 1:length(Condition)
        for t_mode = 1:length(trial_mode)
            ...
            for channel = 1:length(IdChannel)
                ...
                for outcome = 1:length(Outcome)
                    if ~isempty(all_mice_trials.(Outcome{outcome}))
                        if ~isempty(all_mice_trials.(Outcome{outcome}).(Condition{cond}))
                            count = count + 1;
                            [tmp] = [tmp;all_mice_trials.(Outcome{outcome}).(Condition{cond}).(IdChannel{channel}).(trial_mode{t_mode})'];
                            if count == 1
                                mice_limit = size(tmp,1);
                            end
                        end
                    end
                end
                ...
                hold on
                yline(mice_limit+0.5,'w','LineWidth',2)
            ...
            if save_plot == 1
                saveas(gcf,[path2save_figs,'All Mice Heatmaps of dFF signals centered on ',Condition{cond},' ',trial_mode{t_mode},'.jpg'])
                saveas(gcf,[path2save_figs,'All Mice Heatmaps of dFF signals centered on ',Condition{cond},' ',trial_mode{t_mode},'.fig'])
            end
        end
    end
end
% Zscore
if ~isempty(all_mice_zscored_trials)
    Outcome = fieldnames(all_mice_zscored_trials);
    if ~isempty(all_mice_zscored_trials.(Outcome{1}))
        Condition = fieldnames(all_mice_zscored_trials.(Outcome{1}));
    else
        Condition = fieldnames(all_mice_zscored_trials.(Outcome{2}));
    end
    avg_mode = 1; % 1, use mean and SEM; 2, use median and MAD
    for cond = 1:length(Condition)
        for t_mode = 1:length(trial_mode)
            ...
            for channel = 1:length(IdChannel)
                tmp = [];
                for outcome = 1:length(Outcome)
                    if ~isempty(all_mice_zscored_trials.(Outcome{outcome}))
                        if ~isempty(all_mice_zscored_trials.(Outcome{outcome}).(Condition{cond}))
                            count = count + 1;
                            [tmp] = [tmp;all_mice_zscored_trials.(Outcome{outcome}).(Condition{cond}).(IdChannel{channel}).(trial_mode{t_mode})'];
                            if count == 1
            ...
            if save_plot == 1
                saveas(gcf,[path2save_figs,'All trials Heatmaps of Zscored dFF signals centered on ',Condition{cond},' ',trial_mode{t_mode},'.jpg'])
                saveas(gcf,[path2save_figs,'All trials Heatmaps of Zscored dFF signals centered on ',Condition{cond},' ',trial_mode{t_mode},'.fig'])
            end
        end
    end
end
