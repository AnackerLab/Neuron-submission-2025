function plot_z_scored_trials_data_Maryam(Stim_data,t_trials,Outcome,Condition,IdChannel,trial_mode,avg_mode,colors2plot,order_crit,limits2plot,LimitXaxis,Color_scale,show_plot,save_plot,PATH2SAVE)
...
if sum(sum(~isnan((Stim_data.(Condition).corr)))) > 1
    ...
    %     xline(0,'-k');
    if save_plot == 1
        saveas(gcf,[PATH2SAVE,Outcome,' ','Zscored Average signals and moving correlation centered on ',Condition,' ',trial_mode,'.jpg'])
        saveas(gcf,[PATH2SAVE,Outcome,' ','Zscored Average signals and moving correlation centered on ',Condition,' ',trial_mode,'.fig'])
    end
else
    for o = 1:length(IdChannel)
...
if sum(sum(~isnan((Stim_data.(Condition).corr)))) > 1
    ...
    %     xline(0,'-k');
    if save_plot == 1
        saveas(gcf,[PATH2SAVE,Outcome,' ','Heatmaps of Zscored dFF signals and moving correlation centered on ',Condition,' ',trial_mode,'.jpg'])
        saveas(gcf,[PATH2SAVE,Outcome,' ','Heatmaps of Zscored dFF signals and moving correlation centered on ',Condition,' ',trial_mode,'.fig'])
    end
else
    for o = 1:length(IdChannel)
        if ~isempty(IdChannel{o})
