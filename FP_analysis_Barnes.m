...
path2data = 'C:\Users\Anacker1\Desktop\Fiber-Data\Fibers\MH\BARNES MAZE\DAY6-REVERSAL1\'; %Location of the data
path2savefolder = 'C:\Users\Anacker1\Desktop\Fiber-Data\Results\Longer duration\R1\'; %Path to save
...
Animals_dig_correct = [1 2 3 4 5 6]; %% 5 and 6 had it on the left side. Mouse 1 from the first cohort had reward on the right
% if  contains(path2data,'Reversal')
%     Animals_cinnamon_correct = setdiff(1:Nmice,Animals_Digcorrect;
% end

FPrig = 1; % 1 for new rig, 2 for old rig (Pedro's data)
Behav_data = 'Barnes';%'Digging';'Barnes',Ymaze
IdChannel = {'dCA1GCamp6f','vCA1GCamp6f'}; %{'Psychlight','jRGECO1b','GCamp6f'};{'vCA1GCamp6f','dCA1GCamp6f'}'dCA1GCamp6f','vCA1GCamp6f'
color2plotchannels = {'r','g'};%{'r',[],'g'}'r','g'{'g','r'}
MiceperFile = 1;
...
LimitXaxis = [-5 15]; % Window to visualize in the plot. If empty, full length of TRANGE
...
for m = 2
    ...
    % % Define the path to the mouse and find the folders to%%%%:Nmice
    % analyze:Nmice///1
            ...
            for get_behav_data = 1
                if ~isempty(Behav_data)
                    ...
                    elseif strcmp(Behav_data,'Digging')
                        Behavior_code = {'Start','Investcorrect','Investwrong','Digcorrect','Digwrong','PostDec'};%%'IW','DC','DW'
                        Color_code = {'c','r','g','b','y','m'};%
                    elseif strcmp(Behav_data,'Barnes')
                        Behavior_code = {'Wondering','IW','IAC','IC','IAPC','IPC','Inhole'};%%%INVESTWrong= 2/INVEST ADJACENT TO CURREN= 3/INVEST CURRENT TARGET= 4/INVESTIG ADJACENT TO PREVIOUS TARGET= 5/INVESTIGATING PREVIOUS TARGET= 6//7=in hole
                        Color_code = {'c','r','g','b','y','m','k'};%% ned to add 1 more colors, k maybe? black
                    end
                    ...
                    for channel = 1:length(IdChannel)
                        if ~isempty(IdChannel{channel})
                            ...
                            hold on
                            for o = 1:length(Behavior_code)-1
                    ...
                    %%% Zoom in
                    for zoom = 1
                        zoom_limits = [15 26]; % Window to plot zoom of the session (In seconds)
                        figure
                        if show_plot == 0
                            set(gcf,'visible','off')
                        end
                ...
                if ~isempty(IdChannel{o})
                    ...
                    for behav = 1:length(Behavior_code)
                        if ~isempty(Choice_epochs.(Behavior_code{behav}))
                            idx_time = [];
                            for i = 1:size(Choice_epochs.(Behavior_code{behav}),1)
                                [idx_time] = [idx_time,find(time_dwn>= Choice_epochs.(Behavior_code{behav})(i,1) & ...
                                    time_dwn < Choice_epochs.(Behavior_code{behav})(i,2))];
                            end
                            
                            Trial_Measurements.(Outcome_name).norm_AUC.(IdChannel{o}).(Behavior_code{behav}) = ...
                                trapz(tmp(idx_time))./(time_dwn(length(idx_time))- time_dwn(1));
                            Trial_Measurements.(Outcome_name).mean_dFF.(IdChannel{o}).(Behavior_code{behav}) = ...
                                mean(tmp(idx_time),'omitnan');
                            
                            [~,idx_peaks] = intersect(time_dwn(tmp_locs),time_dwn(idx_time));
                            ...
                            Trial_Measurements.(Outcome_name).peaks_rate.(IdChannel{o}).(Behavior_code{behav}) = ...
                                length(idx_peaks)/(time_dwn(length(idx_time))- time_dwn(1));
                            Trial_Measurements.(Outcome_name).mean_peaks_val.(IdChannel{o}).(Behavior_code{behav}) = ...
                                mean(Trial_Measurements.(Outcome_name).peaks.(IdChannel{o}).(Behavior_code{behav}),'omitnan');
                            Trial_Measurements.(Outcome_name).mean_peaks_width.(IdChannel{o}).(Behavior_code{behav}) = ...
                                mean(Trial_Measurements.(Outcome_name).peak_width.(IdChannel{o}).(Behavior_code{behav}),'omitnan');
                            Trial_Measurements.(Outcome_name).mean_peaks_prom.(IdChannel{o}).(Behavior_code{behav}) = ...
                                mean(Trial_Measurements.(Outcome_name).peak_prom.(IdChannel{o}).(Behavior_code{behav}),'omitnan');
                        else
                            Trial_Measurements.(Outcome_name).norm_AUC.(IdChannel{o}).(Behavior_code{behav}) = [];
                            Trial_Measurements.(Outcome_name).mean_dFF.(IdChannel{o}).(Behavior_code{behav}) = [];
                    ...
                    for behav = 1:length(Behavior_code)
                        if ~isempty(Choice_epochs.(Behavior_code{behav}))
                            ...
                            zscored_Trial_Measurements.(Outcome_name).mean_peaks_val.(IdChannel{o}).(Behavior_code{behav}) = ...
                                mean(zscored_Trial_Measurements.(Outcome_name).peaks.(IdChannel{o}).(Behavior_code{behav}),'omitnan');
                            zscored_Trial_Measurements.(Outcome_name).mean_peaks_width.(IdChannel{o}).(Behavior_code{behav}) = ...
                                mean(zscored_Trial_Measurements.(Outcome_name).peak_width.(IdChannel{o}).(Behavior_code{behav}),'omitnan');
                            zscored_Trial_Measurements.(Outcome_name).mean_peaks_prom.(IdChannel{o}).(Behavior_code{behav}) = ...
                                mean(zscored_Trial_Measurements.(Outcome_name).peak_prom.(IdChannel{o}).(Behavior_code{behav}),'omitnan');
                        else
                            zscored_Trial_Measurements.(Outcome_name).norm_AUC.(IdChannel{o}).(Behavior_code{behav}) = [];
                            zscored_Trial_Measurements.(Outcome_name).mean_dFF.(IdChannel{o}).(Behavior_code{behav}) = [];
